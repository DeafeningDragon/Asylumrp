...
function UberEatsDoors(uberChoosenAdress)
    exports[target_name]:AddBoxZone('uber_eats_zone', vec3(Config.UberDelivery[uberChoosenAdress]['x'], Config.UberDelivery[uberChoosenAdress]['y'], Config.UberDelivery[uberChoosenAdress]['z'] - 1), 2.5, 2.5, {
        name = 'uber_eats_zone',
        heading = 90.0,
...
function RemoveUberTarget()
    exports[target_name]:RemoveZone('uber_eats_zone')
end

CreateThread(function()
    exports[target_name]:AddTargetModel(Config.ChatSeller, {
        options = {
            {
                type = 'client',
    ...
    exports[target_name]:AddBoxZone('crypto_spot', vec3(Crypto.Exchange.coords.x, Crypto.Exchange.coords.y, Crypto.Exchange.coords.z), 1.5, 1.5, {
        name = 'crypto_spot',
        heading = 90.0,
        debugPoly = Config.ZoneDebug,
    ...
    exports[target_name]:AddTargetModel('a_m_m_afriamer_01', {
        options = {
            {
    ...
    for k, v in pairs(Config.RentelLocations) do
        exports[target_name]:AddBoxZone('rentel', vec3(v['coords'][1], v['coords'][2], v['coords'][3] - 1), 2.5, 2.5, {
            name = 'rentel',
            heading = 90.0,
            debugPoly = Config.ZoneDebug,
            minZ = v['coords'][3] - 1,
    ...
    exports[target_name]:AddTargetModel(Config.ResetPassword.ped.model, {
        options = {
            {
                type = 'client',
                event = 'phone:client:openChannelMenu',
    ...
    for _, phoneBooth in ipairs(Config.PhoneBooths) do
        exports[target_name]:AddBoxZone('booth_zone', vec3(phoneBooth.coords.x, phoneBooth.coords.y, phoneBooth.coords.z), 1.5, 1.5, {
            name = 'booth_zone',
    ...
    if Config.EnableRecipe then
        for _, recipeShop in ipairs(Config.BuyRecipe.coords) do
            exports[target_name]:AddBoxZone('booth_zone', vec3(recipeShop.x, recipeShop.y, recipeShop.z - 1), 1.5, 1.5, {
                name = 'booth_zone',
...
function InitChargeTarget()
    exports[target_name]:RemoveZone('charger_spots')
    for k, v in pairs(Config.ChargeCoords) do
        local coords = v.coords
        exports[target_name]:AddBoxZone('charger_spots', vec3(coords.x, coords.y, coords.z - 1), 2.5, 2.5, {
            name = 'charger_spots',
            heading = 90.0,
            debugPoly = Config.ZoneDebug,
            minZ = v['coords'][3] - 1,

...
function GetCharName()
    ...
    local p = promise.new()
    TriggerCallback('tk_dispatch:getCharName', function(name)
        p:resolve(name)
    end)
...
function GetPhoneNumber()
    local p = promise.new()
    TriggerCallback('tk_dispatch:getPhoneNumber', function(phone)
        p:resolve(phone)
    end)
    return Citizen.Await(p)
end
...
function GetItemLabel(item)
    local p = promise.new()
    TriggerCallback('tk_dispatch:getItemLabel', function(label)
        p:resolve(label)
    end, item)
    return Citizen.Await(p)
end

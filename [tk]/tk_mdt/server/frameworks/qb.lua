...
function GetProfiles(page)
    ...
    for k,v in pairs(data) do
        data[k].sideBarData = {
            ...
            phoneNumber = page == 'police' and GetPhoneNumber(v.identifier),
...
function GetOfficers()
    ...
    local data = MySQL.Sync.fetchAll([[
        SELECT
            citizenid as identifier,
            CONCAT(JSON_UNQUOTE(JSON_EXTRACT(charinfo, '$.firstname')), ' ', JSON_UNQUOTE(JSON_EXTRACT(charinfo, '$.lastname'))) AS name,
            IF(JSON_VALID(job), JSON_UNQUOTE(JSON_EXTRACT(job, '$.name')), 'Unknown') AS job,
            IF(JSON_VALID(job), JSON_UNQUOTE(JSON_EXTRACT(job, '$.grade.level')), '0') AS grade,
        ...
        WHERE
            FIND_IN_SET(IF(JSON_VALID(job), JSON_UNQUOTE(JSON_EXTRACT(job, '$.name')), ''), ?)
    ]], {policeJobsStr})

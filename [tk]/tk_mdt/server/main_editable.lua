...
function GetPhoneNumber(identifier)
    if Config.Phone == 'lb' then
        return exports["lb-phone"]:GetEquippedPhoneNumber(identifier) or _U('unknown')
...
function SaveProfile(data, identifier)
    MySQL.Async.execute([[
        INSERT INTO
...
function SaveProfileEMS(data, identifier)
    MySQL.Async.execute([[
        INSERT INTO
            tk_mdt_ems_profiles (identifier, image, notes, tags, linkedProfiles, photos)
        VALUES
            (?, ?, ?, ?, ?, ?)
        ON DUPLICATE KEY UPDATE
            image = VALUES(image),
            ...
            linkedProfiles = VALUES(linkedProfiles),
            photos = VALUES(photos)
    ]], {data.identifier, data.image, data.notes, json.encode(data.sideBarData.tags), json.encode(data.sideBarData.linkedProfiles), json.encode(data.sideBarData.photos)})
end
...
function SaveIncident(data)
    MySQL.Async.execute([[
        ...
        WHERE
            id = ?
    ]], {data.title, data.content, json.encode(data.sideBarData.tags), json.encode(data.sideBarData.officers), json.encode(data.sideBarData.criminals), json.encode(data.sideBarData.civilians), json.encode(data.sideBarData.vehicles), json.encode(data.sideBarData.evidence), json.encode(data.sideBarData.weapons), json.encode(data.sideBarData.linkedIncidents), json.encode(data.sideBarData.linkedReports), json.encode(data.sideBarData.linkedPoliceReports), json.encode(data.sideBarData.photos), data.id})
end
...
if Config.Dispatch == 'ps' then
    RegisterServerEvent('ps-dispatch:server:notify', function(data)
        for _,v in pairs(data.jobs) do
            if Config.Jobs.police[v] or v == 'leo' then
...
RegisterCallback('tk_mdt:getIdFromIdentifier', function(src, cb, identifier)
    cb(GetSource(GetPlayerFromIdentifier(identifier)))
end)
...
if Config['Core']:upper() == 'ESX' then
    ...
    function GetItemCount(source, item)
        local xPlayer = GETPFI(source)
        local items = xPlayer.getInventoryItem(item) or exports['codem-inventory']:GetItemsByName(source, item)
        ...
        if items ~= nil then 
            ...
            print("^1PROBLEM!^3 The ^7items ^3are not created. Create them from our documentation. https://docs.brutalscripts.com/site/scripts/mechanic-job/installation-guide")
        end
    end
...
elseif Config['Core']:upper() == 'QBCORE' then
    ...
    function GetItemCount(source, item)
        local xPlayer = GETPFI(source)
        local items = xPlayer.Functions.GetItemByName(item) or exports['codem-inventory']:GetItemsByName(source, item)
